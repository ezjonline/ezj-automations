---
name: os-audit
description: Read-only drift, freshness, and organization audit of the current project's AI OS. Use when the user asks to run an OS audit, check the AIOS for stale or outdated data, verify routing points at things that actually exist, find duplicate or bloated folders, clean up or organize a project, check for context failure modes (poisoning, bloat, confusion, clash), or says "os audit", "is my setup stale", "run a data audit", "my project root is a mess", "my agent keeps missing things that are there", or /os-audit. Do not use for code quality, accessibility, or performance reviews (that is the `audit` skill), for Google Ads account audits (google-ads / ads-audit), for reviewing a git diff (code-review), or for actually applying the fixes (this skill only reports; fixes happen after the user approves).
...
1. Secrets and personal data:
   ...
   - Never print a secret value. Report the file path and the variable NAME only.
...
## Questions for you
{feeds classified RETIRED?, ask "did you stop this on purpose?", plus anything else only the owner can answer}
...
```
/os-audit
/os-audit clients/acme
```

Example of a good finding line, path plus consequence plus tag:

```
- `data/revenue_history.jsonl` last appended 2026-06-30, but CLAUDE.md
  and the daily report both read it as the live revenue source. Any revenue number
  quoted this month is 24 days old. [poisoning]
```

## Hard rules / Never do

- Never fix, move, rename, or delete anything during the audit, even trivial things. The report is the deliverable. Batches A through D wait for the user's explicit yes.
- Never print a secret value. Path and variable name only.
- Never call a feed broken because of an mtime. Verify with content dates or `git log` first (see Notes).
- Never recommend a move for a file that a live script or skill references without listing the referencing paths and marking it move-with-caution.
- Keep the report plain: short sentences, no filler.
- Deliver the summary in chat. The saved .md is the record; the chat summary is what the user reads.

## Notes

- Do not fix anything during the audit, even trivial things. Report first; the fix list is the deliverable.
- Prefer dated filenames and content dates for freshness. File mtimes are the weakest signal: bulk git operations (clone, checkout, big commits) AND cloud-sync tools (OneDrive, Dropbox, Google Drive) can touch every file at once. A project inside a synced folder will show mtime clustering. If many mtimes cluster on one timestamp, verify with content or git log before calling anything "active."
- A stale local snapshot of data whose live source of truth is an external tool (Notion, Airtable, Stripe, Instantly) is a labeling problem, not a freshness problem. The fix is marking it "snapshot, live data lives in X," not endless re-syncing.
...
- Naming note: the global `audit` skill in this setup is the technical quality checker (accessibility, performance, theming) and is NOT the AIS-OS structural companion the original version of this skill referenced. There is no companion installed. This skill stands alone.
